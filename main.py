...
def log_event(attachment, message, person):
    files = {}
...
if __name__ == "__main__":
    ...
    par.add_argument(
        "-C", "--camera", default=source, help="Source of camera or video file path."  # required=True,  # default=2,
    )
    par.add_argument(
    ...
    par.add_argument(
        "--show_detected", default=False, action="store_true", help="Show all bounding box from detection."
    )
    ...
    while cam.grabbed():
        ...
        for i, track in enumerate(tracker.tracks):
            ...
            if track.time_since_update == 0:
                ...
                frame = cv2.putText(
                    frame, str(track_id), (center[0], center[1]), cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 0, 0), 2
                )
        ...
        if event:
            if paused <= 0:
                paused = 120
                image_name = "Screenshots/frame%d.jpg" % f
                cv2.imwrite(image_name, frame)
                log_event(image_name, event, track_id)
            else:
                # print("Paused", paused)
                pass
        event = ""
