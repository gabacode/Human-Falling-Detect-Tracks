...
data_files = ["../Data/Coffee_room_new-set(labelXscrw).pkl", "../Data/Home_new-set(labelXscrw).pkl"]
...
def load_dataset(data_files, batch_size, split_size=0):
    ...
    if split_size > 0:
        ...
        train_set = data.TensorDataset(
            torch.tensor(x_train, dtype=torch.float32).permute(0, 3, 1, 2), torch.tensor(y_train, dtype=torch.float32)
        )
        valid_set = data.TensorDataset(
            torch.tensor(x_valid, dtype=torch.float32).permute(0, 3, 1, 2), torch.tensor(y_valid, dtype=torch.float32)
        )
        train_loader = data.DataLoader(train_set, batch_size, shuffle=True)
        valid_loader = data.DataLoader(valid_set, batch_size)
    else:
        train_set = data.TensorDataset(
            torch.tensor(features, dtype=torch.float32).permute(0, 3, 1, 2), torch.tensor(labels, dtype=torch.float32)
        )
        train_loader = data.DataLoader(train_set, batch_size, shuffle=True)
        valid_loader = None
...
if __name__ == "__main__":
    ...
    train_loader = data.DataLoader(
        data.ConcatDataset([train_loader.dataset, train_loader_.dataset]), batch_size, shuffle=True
    )
    ...
    plot_confusion_metrix(
        ...
        y_preds,
        class_names,
        "Eval on: {}\nLoss: {:.4f}, Accu{:.4f}".format(os.path.basename(data_file), run_loss, run_accu),
        "true",
        save=os.path.join(save_folder, "{}-confusion_matrix.png".format(os.path.basename(data_file).split(".")[0])),
    )
