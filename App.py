...
class Models:
    ...
    def load_models(self):
        ...
        self.pose_model = SPPE_FastPose(self.pose_backbone, self.inp_pose[0], self.inp_pose[1], device=self.device)
    ...
    def process_frame(self, frame):
        ...
        for i, track in enumerate(self.tracker.tracks):
            ...
            if track.time_since_update == 0:
                ...
                frame = cv2.putText(
                    frame, str(track_id), (center[0], center[1]), cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 0, 0), 2
                )
...
class main:
    def __init__(self, master: tk.Tk):
        ...
        self.cam = None
        self.canvas = tk.Canvas(master, width=int(self.width * 0.65), height=self.height)
    ...
    def update(self):
        ...
        if self.cam.grabbed():
            ...
            frame = cv2.resize(
                frame, (self.canvas.winfo_width(), self.canvas.winfo_height()), interpolation=cv2.INTER_CUBIC
            )
